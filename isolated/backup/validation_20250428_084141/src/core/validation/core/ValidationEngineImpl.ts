...
import { IValidationEngineWithHooks, ValidationEngineOptions, ValidationEngineHooks, IValidationEngine } from "./IValidationEngine";
...
export class ValidationEngineImpl implements IValidationEngineWithHooks {
  ...
  constructor(
    ...
    this.options = {
      ...
      componentName: options.componentName || 'ValidationEngine',
  ...
  public validateNode(node: any): ValidationResult<any> {
    ...
    try {
      ...
      return ValidationResult.createInvalid(validationError);
    }
  }
   ...
   * @returns This engine instance for chaining
   */
  public registerRules(rules: ValidationRule[]): IValidationEngine {
   ...
   * @returns This engine instance for chaining
   */
  public optimize(): IValidationEngine {
   ...
   */
  public traverseNode(node: any, result: ValidationResult): void {
    if (!node) return;
   ...
   * @returns Validation result
   */
  public processRules(node: any, rules: ValidationRule[]): ValidationResult {
    // Create a new result
