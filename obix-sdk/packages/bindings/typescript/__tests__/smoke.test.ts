...
import { createTypeScriptBinding } from '../src/index';

describe('typescript binding smoke', () => {
  it('toggles initialize/destroy state and uses shared invocation envelope', async () => {
    const ffiPath = '/tmp/obix-typescript-ffi.mock';

    const binding = createTypeScriptBinding({
      ffiPath,
      schemaMode: 'hybrid',
      memoryModel: 'hybrid',
      runtimeType: 'node',
    ...
    expect(result).toMatchObject({
      ...
      echo: {
        functionId: 'ping',
        args: [1, 2, 3],
        metadata: { binding: 'typescript', ffiPath },
      },
    });
