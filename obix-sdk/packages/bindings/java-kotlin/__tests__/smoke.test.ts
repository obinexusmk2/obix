...
import { createJavaKotlinBinding } from '../src/index';

describe('java-kotlin binding smoke', () => {
  it('toggles initialize/destroy state and uses shared invocation envelope', async () => {
    const ffiPath = '/tmp/obix-java-kotlin-ffi.mock';
    ...
    expect(result).toMatchObject({
      ...
      echo: {
        functionId: 'ping',
        args: [1, 2, 3],
        metadata: { binding: 'java-kotlin', ffiPath },
      },
    });
