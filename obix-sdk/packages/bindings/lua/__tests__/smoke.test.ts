...
import { createLuaBinding } from '../src/index';

describe('lua binding smoke', () => {
  it('toggles initialize/destroy state and uses shared invocation envelope', async () => {
    const ffiPath = '/tmp/obix-lua-ffi.mock';

    const binding = createLuaBinding({
      ffiPath,
      schemaMode: 'hybrid',
      memoryModel: 'hybrid',
