...
import type { ComponentLogic, ActionContext, ReactiveComponent, Action } from "./types";
 ...
 */
export class ReactiveWrapper<S extends Record<string, any> = Record<string, any>>
  implements ReactiveComponent<S>
{
  state: S;
  ...
  constructor(logic: ComponentLogic<S>) {
    ...
    this.proxyState = new Proxy(this.state, {
      set: (target, property, value) => {
        target[property as keyof S] = value;
        this.notify();
        return true;
      },
    });
